...
void kernel rayGenerator(
	...
	rayTree.reflectFactor = vertex.reflectFactor;
	rayTree.refractFactor = vertex.refractFactor;
	...
	Hit hit = hits[gid];
	Ray reflection = genReflectionRay(hit);
	Ray refraction = genRefractionRay(hit);
...
void summarizeRays(global Hit* results, global atomic_int* globalResultCount, Hit result, bool hasResult, int* indexOut, local atomic_int* groupResultCount){
